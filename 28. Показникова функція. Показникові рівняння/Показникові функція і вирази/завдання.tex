...
\noindent{\small\color{gray!80} Розділ 3. Функції \quad\textbullet\quad Усього завдань: 31 (НМТ 2023~--- 16, 2024~--- 8, 2025~--- 7).}\par\vspace{0.2cm}
...
\noindent\zadnum Графік функції \colorbox{yellow!30}{$y = 3^x$} паралельно перенесли на 2 одиниці вниз уздовж осі $y$. Укажіть функцію, графік якої отримали в результаті цього перетворення. \nmtyear{2025}
\vspace{0.3cm}

\answerTable{$y = 3^{x-2}$}{$y = 3^x - 2$}{$y = 3^{x+2}$}{$y = 3^x + 2$}{$y = \displaystyle\frac{3^x}{2}$}

\noindent\zadnum Спростіть вираз \colorbox{yellow!30}{$5^{1+x} \cdot 5^{1-x}$}. \nmtyear{2025}
\vspace{0.3cm}
