...
\begin{samepage}
\zadtask{Установіть відповідність між твердженням <<областю визначення функції є проміжок $[-1; +\infty)$>> та функцією \mbox{(А--Д)}, для якої це твердження є правильним. \nmtyear{2023}}
...
\begin{samepage}
\zadtask{Установіть відповідність між твердженням <<функція має 2 нулі>> та функцією \mbox{(А--Д)}, для якої це твердження є правильним. \nmtyear{2024}}
...
\noindent\zadnum На кожному з рисунків <<Рис. 1>> зображено графік функції $y=f(x)$, визначеної на проміжку $[-3; 3]$. Узгодьте рисунок \mbox{(1--3)} з твердженням \mbox{(А--Д)} щодо функції $y=f(x)$, графік якої зображено на цьому рисунку. \nmtyear{2024}
...
\noindent
\par\nopagebreak\vspace{0.15cm}
\answerRows{графік функції $f$ є фрагментом кола $x^2 + (y-1)^2 = 9$}{графік функції $f$ є фрагментом кола $x^2 + (y+1)^2 = 9$}{функція $f$ зростає на області визначення}{графік функції $y=f(x)+2$ розташований лише в I і II чвертях}{графік функції $f$ має лише одну спільну точку з графіком функції $y=2^x$}
\par\penalty-20
\end{samepage}
...
% рік визначено за сусідніми завданнями (у старому файлі тег року відсутній)
\noindent\zadnum \begin{minipage}[t]{0.55\textwidth}
На рисунку зображено графік функції $y=f(x)$, визначеної на проміжку $[-4; 6]$. Узгодьте проміжок $[-4; 0]$ із функцією \mbox{(А--Д)}, що описує графік функції $y=f(x)$ на цьому проміжку. \nmtyear{2025}
\end{minipage}
\hfill
\begin{minipage}[t]{0.4\textwidth}
    \nopagebreak\nbvspace{-0.3cm}
...
\noindent
\par\nopagebreak\vspace{0.15cm}
\answerRows{$y=x^3$}{$y=12-2x$}{$y=-\sqrt{x}$}{$y=2^x$}{$y=\sqrt{-x}$}
\par\penalty-20
\end{samepage}
...
\begin{samepage}
% Відповідь: Д
\zadtask{Узгодьте вираз $\sqrt{\left(2^a\right)^3}$ із його значенням \mbox{(А--Д)}, якщо $a = \dfrac{2}{3}$. \nmtnobreak \nopagebreak\nbvspace{0.3cm} \noindent \matchingLayout{ \matchHead{Вираз} \matchItem{1}{$6a$} \matchItem{2}{$\sqrt{\left(2^a\right)^3}$} \matchItem{3}{$\log_3 (1 - a)$} }{ \matchHead{Значення виразу} \matchItem{А}{$4$} \matchItem{Б}{$-1$} \matchItem{В}{$1$} \matchItem{Г}{$\dfrac{20}{3}$} \matchItem{Д}{$2$} }{ \matchingGrid } \nmtyear{2026}}
...
\begin{samepage}
% Відповідь: В
\zadtask{На рисунку зображено графік функції $y = f(x)$, визначеної на проміжку $(-\infty;\, +\infty)$. Узгодьте проміжок $(-\infty;\, -1)$ із функцією \mbox{(А--Д)}, що описує графік функції $y = f(x)$ на цьому проміжку. \nmtyear{2026}}
\answerRows{$y = \cos x$}{$y = 2^x$}{$y = -\dfrac{2}{x}$}{$y = \sin x$}{$y = -2x$}
\par\penalty-20
\end{samepage}
